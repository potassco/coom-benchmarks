...
parser.add_argument(
    "--maxrooms", type=int, default=10, help="Upper bound of the domain for numRooms"
)
parser.add_argument(
    "--maxthings", type=int, default=10, help="Upper bound of the domain for numThings"
)
...
parser.add_argument(
    "--instances", type=int, default=10, help="Number of instances to generate"
)
parser.add_argument(
    "--randomize",
    type=int,
...
parser.add_argument(
    "--instancedir", type=str, default="instances", help="Instance output directory"
)
parser.add_argument(
    "--modeldir", type=str, default="model", help="Model output directory"
)
...
def get_user_input(
    rooms: Optional[int], persons: int, short_things: int, long_things: int
) -> List[str]:
    user_input = []
...
if __name__ == "__main__":
    ...
    # Write COOM model of instance
    # os.makedirs(args.modeldir, exist_ok=True)

    model = get_instance(args.maxrooms, args.maxthings, args.persons)

    modelname = f"{args.name}-{args.maxrooms}-{args.maxthings}-{args.persons}" + (
        f"_{args.rooms}" if args.rooms else ""
    )
    outmodel = os.path.join(args.modeldir, modelname + ".coom")
    with open(outmodel, "w", encoding="utf-8") as f:
        f.write(model)

    # Write COOM user input of instance
    ...
    for i, things in enumerate(num_things):
        ...
        for j, (short_things, long_things) in enumerate(pairs):
            user_input = get_user_input(
                args.rooms, args.persons, short_things, long_things
            )
            userinputname = f"{modelname}_user-input-{things}" + (
                f"-{j}" if args.randomize else ""
            )
            outuser = os.path.join(args.instancedir, userinputname + ".coom")
