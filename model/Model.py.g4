...
behavior:
	'behavior' name? behavior_block;

behavior_block:
	'{'
		(define stmt_end)*
		(message (stmt_end message)*)?
		(conditioned (stmt_end conditioned)*)?
	'}' ;

feature: ref='reference' ? cardinality ? field ('/' priority=INTEGER)?;
cardinality: min=(INTEGER | TIMES) (range='..' (max=(INTEGER | TIMES | '*'))?)? ;

attribute: 'attribute' field ;
option: name ('=' '(' constant (','? constant)* ')')? ;

field: (number_def fieldName=name) | (string_def fieldName=name) | (type_ref=name ? fieldName=name) ;
number_def: 'num' fraction? unit? (min=floating '-' max=floating)? ;
string_def: 'string' multiLine='/n' ? maxLength=INTEGER ? ;

define: 'define' name '=' formula ;
conditioned:
    (explanation stmt_end)?
    (index_quantification stmt_end)?
	...
	( behavior_block | assign_default | assign_imply | assign_new | interaction | require | prefer | combinations );
...
index_quantification: ('forall'|'exists') index_variable=NAME (','? index_variable=NAME)*;
precondition: 'condition' condition | 'valid' (('start' start=date ('end' end=date)?) | ('end' end=date)) ;
assign_default: 'default' path '=' formula ;
assign_imply: 'imply' path '=' formula ;
assign_new: 'new' path ;
interaction: directive=('readwrite'|'readonly'|'hide') ('/' priority=INTEGER)? path ;
message: (precondition stmt_end)* 'message' ('/' level=('debug'|'info'|'warn'|'error'|'explain'))? name ;
require: 'require' condition ;
prefer: 'prefer' ('/' penalty=INTEGER)? condition ;
combination_row: rowType=('allow'|'forbid') '(' combination_item (','? combination_item)* ')' ;
combination_item : any='-*-' | '(' combination_atom (','? combination_atom)* ')' | combination_atom ;
combination_atom : operator=compare? formula;

stmt_end: ';' | {self.wasNewline()} ;

constant: floating | name ;
floating: '-'? (FLOATING | INTEGER | '\u221e') ; // == infinity symbol
//integer: '-'? INTEGER ;
compare : '<'|'<='|'≤'|'>'|'>='|'≥'|'='|'=='|'!='|'≠'|'⊇'|'contains' ;

unit: '/' text=(NAME | FLOATING | INTEGER | TIMES | HASHES | FUNCTION) ;
fraction: sign=('.' | '^') digits=HASHES ;

// define path expressions
path: path_item ('.' path_item)* ;
path_item: name ('[' path_index ('..' path_index)? ']')? ;
path_index: index_variable=NAME | INTEGER | ('last' ('-' INTEGER)?) ;

// define conditions
condition: condition_or ;
condition_or: condition_and ('||' condition_and)* ;
condition_and: condition_not ('&&' condition_not)* ;
condition_not: '!' condition_not | ('(' condition ')') | condition_compare ;
...
KEYWORD : 'last'|'true'|'false'
	|'condition'|'valid'|'start'|'end'
	|'product'|'structure'|'enumeration'|'behavior'
	|'default'|'imply'|'new'|'require'|'combinations'|'allow'|'forbid'
	|'readwrite'|'readonly'|'hide'
	|'attribute'|'reference' ;
NAME : (ALPHA ALPHANUMERIC*) | QUOTED_SINGLE | QUOTED_DOUBLE ;

fragment ALPHANUMERIC: ALPHA | DIGIT ;
fragment ALPHA: [_a-zA-Z$\u00A2-\u00A5\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u02FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FEF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD] ;
fragment DIGIT: [0-9] ;
