...
They are modular in the sense that the `clingo-part.lp` encoding
imports all rules from the `clingo-core.lp` encoding and the `clingo-num.lp`
imports all rules from the `clingo-part.lp` encoding.

The `encodings/clingo-core.lp` contains all features of the kids bike.
To run
```
clingo encodings-clingo-core.lp examples/asp/kids-bike.lp
```

Similarly, for the city and travel bike run
```
clingo encodings/clingo-part.lp examples/asp/city-bike.lp
```
```
clingo encodings/clingo-num.lp examples/asp/travel-bike.lp
```

> *Note:* Due to the modularity the encodings are downward compatible, i.e.,
you can solve the kids bike with the `clingo-part.lp` or `clingo-num.lp` as well.
...
```
fclingo encodings/fclingo-core.lp examples/asp/kids-bike.lp 0
fclingo encodings/fclingo-part.lp examples/asp/city-bike.lp 0
fclingo encodings/fclingo-num.lp examples/asp/travel-bike.lp 0
```
